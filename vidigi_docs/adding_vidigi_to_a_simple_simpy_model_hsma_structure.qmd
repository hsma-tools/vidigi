---
title: Adding Vidigi to a Simple simpy Model (HSMA Structure) - vidigi 2.0.0 and above
execute:
  eval: false
format:
  html:
...
filters:
  - quarto
  - line-highlight
image: adding_vidigi_to_a_simple_simpy_model_hsma_structure.gif
pub-info:
    abstract: |
        A step-by-step walkthrough of retrofitting vidigi onto an existing SimPy model written in the
        style taught on the Health Service Modelling Associates (HSMA) programme. The core steps -
        adding logging, swapping in vidigi's resource classes, and building the animation - apply
        just as well if your model is structured differently.
---
...
While vidigi has been tested to ensure that it's special resource classes work the same as existing simpy resource classes, it's still possible to accidentally change your model. There's also a chance that the vidigi classes don't work identically to simpy classes in more complex scenarios with reneging, baulking, or other conditional logic around resource allocation - while we are trying to expand our tests to cover these edge cases, there's always a risk that you're doing something in a way we've not come across yet.
...
However, the `vidigi.ciw` module's `event_log_from_ciw_recs` function provides a simple way to get the required logs out of your ciw model without any additional logging being added in manually. Its siblings `event_logger_from_ciw_recs` and `trial_logger_from_ciw_recs` return an `EventLogger` / `TrialLogger` instead, if you want vidigi's post-run analysis tools as well.
...
We will also pass in our event logger, which will allow resource use start and end to be written.
...
We are also going to pass in the logger to this resource, which will allow logs to automatically be added in.
...
This is the key place in which we add our remaining *logging* - arrivals, queues, and departures. The logs are what vidigi relies on to calculate who should be where, when, within the animation.

This is also where we also need to slightly change the way we request the resource to ensure the resource use events are logged.
...
- when they are using a resource (done automatically if using a VidigiStore and passing in a logger and a label, and an entity_id when requesting)
- when they finish using that resource (as above)
- when they leave

You can have multiple instances of queues and resource use within your logs per entity.

**However, each entity should have only one arrival and one departure.** Reusing an entity ID for a second arrival or departure averages the duplicate times together instead of keeping either one; vidigi now raises a `ValueError` naming the offending entity ID when this happens, rather than silently producing a wrong animation.
...
We'll now make use of our `TrialLogger` class to store all `EventLogger` objects from each run of the model, allowing us to access a range of additional helper methods.
...
def run_trial(self):
    for run in range(1, g.number_of_runs+1):
        ...
        # For each run, we add the log to the TrialLogger object    #<<
        self.trial_log.add_log(my_model.logger) #<<
...
:::{.callout-warning}
If you skip an event that turns out to matter, the symptom is worse than a missing label: any entity whose current step *is* that event when a frame is drawn simply vanishes for that frame, then reappears - flying in from the top-left corner - once a positioned event takes over again. `resource_use_end` gets away with being skipped because it's almost always logged at the exact same instant as the next step, so it's never actually the "current step" shown in any frame - that's a coincidence of how it's logged, not a guarantee vidigi makes. `depart` doesn't get that same protection, so leaving it unpositioned is a common way to hit this by accident. `generate_animation_df` now warns automatically whenever this actually happens, naming the event responsible.
:::
...
- Via `TrialLogger.animate_activity_log()`
- Via `EventLogger.animate_activity_log()`
- Called directly via `from vidigi.animation import animate_activity_log` and then call the function as `animate_activity_log(event_log=...)` where event_log is either
...
:::{.callout-warning}
If you go for option 3 - passing in a dataframe representing the event log - is important that you only pass in a single run at a time!

Passing a dataframe in containing more than one run will produce incorrect animations.

You may, however, wish to give the user control over which run they visualise using a dropdown in something like Streamlit or Shiny, filtering the event log on the fly (though you could also achieve this by varying the run number parameter in conjunction with a TrialLogger).
:::

```{python}
# Filter our dataframe down to a single run
my_model.trial_log.animate_activity_log(
...
class Model:
    ...
    def attend_clinic(self, patient):
        ...
        # Seize a treatment resource when available
        with self.treatment_cubicles.request(entity_id=patient.identifier) as req: #<< Pass the patient ID
...
class Trial:
    ...
    def run_trial(self):
        for run in range(1, g.number_of_runs+1):
            ...
            # For each run, we append the logger object (which is of class  #<<
            # EventLogger) to our list all_event_logs, which started out empty  #<<
            self.trial_log.add_log(my_model.event_log)
...
Let's have a look at what our generated event logs look like.

```{python}
#| eval: true
my_trial.trial_log.get_log_by_run(run_number=1, as_df=True).head(10)
```
...
#| eval: true
#| echo: false
my_trial.trial_log.animate_activity_log(
    # Choose the run we want to animate
    run_number=1,
    # Pass in our event position dataframe
    event_position_df=event_position_df,
    # Use an instance of the g class as our scenario so that it can access the
    # required information about how many resources are available
    scenario=g(),
    # How long should the animation last? We can pass in any value here - but
    # I've chosen to make it last as long as our originally defined simulation
    # duration
    limit_duration=g.sim_duration,
    # Turn on logging messages
    debug_mode=True,
    # Turn on axis units - this can help with honing your event_position_df
    # iteratively
    setup_mode=True,
    # How big should the time steps be? Here,
    every_x_time_units=1,
    # Should the animation allow you to just drag a slider to progress through
    # the animation, or should it include a play button?
    include_play_button=True,
    # How big should the icons representing our entities be?
    entity_icon_size=20,
    # How big should the icons representing our resources be?
    ...
    # How big should the gap between our entities be when they are queueing?
    gap_between_entities=6,
    # When we wrap the entities to fit more neatly on the screen, how big
    # should the vertical
    # gap be between these rows?
    gap_between_queue_rows=25,
    # How tall, in pixels, should the plotly plot be?
    ...
    # How long, in milliseconds, should each frame last?
    frame_duration=200,
    # How long, in milliseconds, should the transition between each pair of
    # frames be?
    frame_transition_duration=600,
    # How wide, in coordinates, should our plot's internal coordinate system
    # be?
    override_x_max=300,
    # How tall, in coordinates, should our plot's internal coordinate system
    # be?
    override_y_max=500,
    # How long should a queue be before it starts wrapping vertically?
    wrap_queues_at=25,
    # What are the maximum numbers of entities that should be displayed in any
    # queueing steps before displaying additional entities as a text string
    # like '+ 37 more'
    step_snapshot_max=125,
    # What should the time display units be underneath the simulation?
    time_display_units="simulation_day_clock_ampm",
    # display our Label column from our event_position_df to identify the
...
animate_activity_log(
    ...
    step_snapshot_max=125,
    time_display_units="simulation_day_clock_ampm",
    display_stage_labels=False,  # hide Label column from event_position_df
    # Add a local or web-hosted image as our background
    add_background_image=(
        "https://raw.githubusercontent.com/hsma-tools/vidigi/refs/heads/main/"
        "examples/example_1_simplest_case/Simplest%20Model%20Background%20"
        "Image%20-%20Horizontal%20Layout.drawio.png"
    )
)
```
