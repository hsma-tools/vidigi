...
We are also going to pass in the logger to this resource, which will allow logs to automatically be added in when entities begin using resources, and when they finish, automatically capturing the relevant ID of the resource.
...
```{python}
#| eval: true
my_trial.trial_log.get_log_by_run(run=1, as_df=True).head(10)
```
