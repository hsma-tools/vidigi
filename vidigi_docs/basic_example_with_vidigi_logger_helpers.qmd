...
We can take a look at the dataframe created with the `.to_dataframe` method.

```{python}
event_log.to_dataframe().head()
```
...
- Via `TrialLogger.animate_activity_log()` (not covered here)
...
::: {.column width='50%'}
### Calling as a method of the EventLogger
