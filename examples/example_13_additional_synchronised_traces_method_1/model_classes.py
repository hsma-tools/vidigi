...
class Schedule:
    ...
    def create_schedule(
        self, weekday, sessions_per_weekday_list, allocation, theatres_per_weekday
    ):
        """
        Arguments needed:
    ...
    def week_schedule(self):
        ...
        day_data = self.create_schedule(
            self.weekday,
            self.sessions_per_weekday_list,
            self.allocation,
            self.theatres_per_weekday,
        )
    ...
    def theatre_capacity(self):
        length_sched = int(
            round(
                2 * (DEFAULT_WARM_UP_PERIOD + DEFAULT_RESULTS_COLLECTION_PERIOD) / 7, 0
            )
        )
        ...
        for week in range(length_sched):
            ...
            DEFAULT_SCHEDULE_AVAIL = pd.concat(
                [DEFAULT_SCHEDULE_AVAIL, single_random_week], axis=0
            )
...
los_data = pd.read_csv(
    "https://raw.githubusercontent.com/AliHarp/HEP/main/HEP_notebooks/01_model/Test_data/los_spells_no_zero.csv"
)
...
class Scenario:
    ...
    def init_sampling(self):
        ...
        self.primary_hip_dist = Lognormal(
            self.primary_hip_mean_los,
            self.primary_hip_sd_los,
            random_seed=self.seeds[0],
        )
        self.primary_knee_dist = Lognormal(
            self.primary_knee_mean_los,
            self.primary_knee_sd_los,
            random_seed=self.seeds[1],
        )
        self.revision_hip_dist = Lognormal(
            self.revision_hip_mean_los,
            self.revision_hip_sd_los,
            random_seed=self.seeds[2],
        )
        self.revision_knee_dist = Lognormal(
            self.revision_knee_mean_los,
            self.revision_knee_sd_los,
            random_seed=self.seeds[3],
        )
        self.unicompart_knee_dist = Lognormal(
            self.unicompart_knee_mean_los,
            self.unicompart_knee_sd_los,
            random_seed=self.seeds[4],
        )

        # Empirical LoS distributions for each surgery patient type
        self.primary_hip_dist_emp = Empirical(
            self.primary_hip_los_data, random_seed=self.seeds[5]
        )
        self.primary_knee_dist_emp = Empirical(
            self.primary_knee_los_data, random_seed=self.seeds[6]
        )
        self.revision_hip_dist_emp = Empirical(
            self.revision_hip_los_data, random_seed=self.seeds[7]
        )
        self.revision_knee_dist_emp = Empirical(
            self.revision_knee_los_data, random_seed=self.seeds[8]
        )
        self.unicompart_knee_dist_emp = Empirical(
            self.unicompart_knee_los_data, random_seed=self.seeds[9]
        )

        # distribution for delayed LoS
        self.los_delay_dist = Lognormal(
            self.delay_post_los_mean, self.delay_post_los_sd, random_seed=self.seeds[10]
        )
        ...
        if self.need_for_los_delay:
            ...
            with self.args.beds.get() as req:
                ...
                if req in admit:
                    ...
                    trace(
                        f"primary patient {self.id} {self.primary_label}"
                    ...
                    trace(
                        f"los of primary patient {self.id} completed at {self.env.now:.3f}"
                    )
        ...
        # Pathway for no delayed los
            ...
            with self.args.beds.get() as req:
                ...
                if req in admit:
                    ...
                    self.depart = self.env.now
                    trace(
                        f"los of primary patient {self.id} {self.primary_label}"
        ...
        if self.need_for_los_delay:
            ...
            with self.args.beds.get() as req:
                ...
                if req in admit:
                    ...
                    self.queue_beds = self.env.now - self.arrival
                    trace(
                        f"revision patient {self.id} {self.revision_label}"
                        f"has been allocated a bed at {self.env.now:.3f}"
                        f"and queued for {self.queue_beds:.3f}"
                    )
                    self.event_log.append(
                    ...
                    self.depart = self.env.now
                    trace(
                        f"los of revision patient {self.id} {self.revision_label}"
                        f"completed at {self.env.now:.3f}"
                    )
                    self.total_time = self.env.now - self.arrival
                    trace(
        ...
        # no need for delayed discharge
            ...
            with self.args.beds.get() as req:
                ...
                if req in admit:
                    ...
                    trace(
                        f"los of revision patient {self.id} completed at {self.env.now:.3f}"
                    )
                    self.total_time = self.env.now - self.arrival
                    trace(
                        f"revision patient {self.id} total los = {self.total_time:.3f}"
                    )
                    self.event_log.append(
                        {
                            "patient": self.id,
...
class Hospital:
    ...
    def perform_audit(self):
        ...
        while True:
            ...
            primary_beds = (
                self.args.beds.capacity - len(self.args.beds.items)
            ) in self.cum_primary_patients
            (self.audit_beds_used_primary.append(primary_beds / self.args.n_beds))

            revision_beds = (
                self.args.beds.capacity - len(self.args.beds.items)
            ) in self.cum_revision_patients
            (self.audit_beds_used_revision.append(revision_beds / self.args.n_beds))

            (
                self.audit_beds_used.append(
                    (self.args.beds.capacity - len(self.args.beds.items))
                    / self.args.n_beds
                )
            )
    ...
    def run(
        self,
        results_collection=DEFAULT_RESULTS_COLLECTION_PERIOD + DEFAULT_WARM_UP_PERIOD,
    ):
        """
        single run of model
        """
    ...
    def patient_arrivals_generator_primary(self):
        ...
        for day in range(len(sched)):
            ...
            trace(
                f"--------- {primary_arrivals} primary patients are scheduled on Day {day} -------"
            )
            for i in range(primary_arrivals):
                new_primary_patient = PrimaryPatient(
                    day=day,
                    id=pt_count,
                    env=self.env,
                    args=self.args,
    ...
    def patient_arrivals_generator_revision(self):
        ...
        for day in range(len(sched)):
            ...
            trace(
                f"--------- {revision_arrivals} revision patients are scheduled on Day {day} -------"
            )
            for i in range(revision_arrivals):
                new_revision_patient = RevisionPatient(
                    day=day,
                    id=pt_count,
                    env=self.env,
                    args=self.args,
