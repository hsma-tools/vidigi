...
class Summary:
    ...
    def process_run_results(self):
        ...
        patients = len(
            [
                p
                for p in self.model.cum_primary_patients
                if p.day > DEFAULT_WARM_UP_PERIOD
            ]
        ) + len(
            [
                p
                for p in self.model.cum_revision_patients
                if p.day > DEFAULT_WARM_UP_PERIOD
            ]
        )

        primary_arrivals = len(
            [
                p
                for p in self.model.cum_primary_patients
                if p.day > DEFAULT_WARM_UP_PERIOD
        ...
        primary_throughput = len(
            [
                p
                for p in self.model.cum_primary_patients
                if (p.total_time > -np.inf) & (p.day > DEFAULT_WARM_UP_PERIOD)
            ]
        )
        revision_throughput = len(
            [
                p
                for p in self.model.cum_revision_patients
                if (p.total_time > -np.inf) & (p.day > DEFAULT_WARM_UP_PERIOD)
        ...
        mean_primary_queue_beds = np.array(
            [
                p.queue_beds
                for p in self.model.cum_primary_patients
        ...
        mean_revision_queue_beds = np.array(
            [
                p.queue_beds
                for p in self.model.cum_revision_patients
                if p.queue_beds > -np.inf
        ...
        los_primary = np.array(
            [
                p.primary_los
                for p in self.model.cum_primary_patients
                if (p.primary_los > -np.inf) & (p.day > DEFAULT_WARM_UP_PERIOD)
            ]
        ).sum()
        mean_primary_bed_utilisation = los_primary / (
            DEFAULT_RESULTS_COLLECTION_PERIOD * self.args.n_beds
        )
        los_revision = np.array(
            [
                p.revision_los
                for p in self.model.cum_revision_patients
                if (p.revision_los > -np.inf) & (p.day > DEFAULT_WARM_UP_PERIOD)
            ]
        ).sum()
        mean_revision_bed_utilisation = los_revision / (
            DEFAULT_RESULTS_COLLECTION_PERIOD * self.args.n_beds
        )
