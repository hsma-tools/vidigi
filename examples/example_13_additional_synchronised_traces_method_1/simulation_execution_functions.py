...
def single_run(
    scenario, results_collection=42 + 7, random_no_set=None, return_detailed_logs=False
):
    """
...
def multiple_replications(
    scenario, results_collection=42, warmup=7, n_reps=30, return_detailed_logs=False
):
    """
    ...
    results = [
        single_run(scenario, results_collection_plus_warmup, random_no_set=rep)[0]
    ...
    day_results = [
        single_run(scenario, results_collection_plus_warmup, random_no_set=rep)[1]
        for rep in range(n_reps)
    ]
    ...
    primary_pt_results = [
        single_run(scenario, results_collection_plus_warmup, random_no_set=rep)[2][
            0
        ].assign(rep=rep + 1)
        for rep in range(n_reps)
    ]
