...
class Scenario:
    ...
    def __init__(
        ...
        self.priority_dist = Bernoulli(PROP_HIGH_PRORITY, random_seed=self.seeds[1])
        ...
        for i in range(len(self.clinic_demand)):
            clinic = Clinic(
                self.clinic_demand["referred_out"].iloc[i],
...
class PatientReferral:
    ...
    def __init__(
        self, env, args, referral_t, home_clinic, booker, event_log, identifier
    ):
        self.env = env
        self.args = args
    ...
    def execute(self):
        ...
        best_t, self.booked_clinic = self.booker.find_slot(
            self.referral_t, self.home_clinic
        )
