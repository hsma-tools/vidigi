...
class PackingRobot:
    ...
    def _with_packing_offset(self, base_pos):
        ...
        if base_pos == LAYOUT["packing"]:
            return (
                base_pos[0] + self.packing_offset[0],
    ...
    def _logical_pos(self, pos):
        ...
        if (
            abs(pos[0] - px - self.packing_offset[0]) < 1e-6
            and abs(pos[1] - py - self.packing_offset[1]) < 1e-6
        ):
        ...
        for axis, delta in sequence:
            if delta != 0:
                ...
                if remaining > 0:
                    ...
                    if (
                        self._logical_pos(self.pos) != LAYOUT["packing"]
                        and self._logical_pos(next_pos) != LAYOUT["packing"]
                    ):
                        while any(
                            other != self.name
                            and abs(robot_positions[other][0] - next_pos[0]) < 1e-6
                            and abs(robot_positions[other][1] - next_pos[1]) < 1e-6
...
if __name__ == "__main__":
    ...
    robots = [
        PackingRobot(env, f"RoboPack-{i + 1}", logger, packing_slot=i) for i in range(8)
    ]
    for robot in robots:
