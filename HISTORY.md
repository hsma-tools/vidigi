...
- New `vidigi.analysis.resource_utilisation(event_log, by=..., ...)`, aggregating those intervals into busy time, mean-in-use and utilisation — always one row per run per group, matching `replication_means`'s "aggregation across runs is the plotting layer's job" convention
    ...
    - `TrialLogger.get_resource_utilisation()` and `TrialLogger.plot_resource_utilisation()` gained the same `resource_col_name=` parameter (default `"resource_id"`, matching current behaviour exactly). Previously only the free `vidigi.analysis.resource_utilisation`/`vidigi.plots.plot_resource_utilisation` functions accepted it, so the `unique_resource_id` pattern above only worked by dropping down to those directly rather than through `TrialLogger`'s own convenience API
...
Test coverage grew from 31 to 547 tests, concentrated on the parts of the pipeline where a
...
- `TrialLogger.get_resource_utilisation()`'s and `.plot_resource_utilisation()`'s new `resource_col_name=` passthrough is covered against a two-entity fixture that genuinely collides under the default `resource_id` column but resolves cleanly under a `unique_resource_id` column pointed at directly — mutation-proven to fail if either method's forwarding were dropped
