...
- Versions with breaking changes open with `### ⚠️ Breaking changes` (scannable one-liners); the detail bullets below it go in `### Notes` if the release is small, or once there are enough bullets to make one flat list unwieldy, split by nature of change instead — `### New features` / `### Fixes` / `### Deprecations` (plus `### New examples` if relevant) — see 1.4.0. A version with no breaking changes uses the same grouping option, or a plain flat list (1.3.1), whichever suits the release.
- If `### New features` itself grows large enough that scanning it is slow — e.g. a release adding several `vidigi.analysis` functions alongside several `vidigi.plots`/`TrialLogger.plot_*` ones — split it further into `### New metrics` (analysis functions and numbers-only `TrialLogger` delegators) and `### New plots` (chart-returning functions), leaving non-analysis/plotting additions (new animation arguments, new warnings, logging API changes, typing) in `### New features`. See 1.4.0. A bullet introducing an analysis function and its plotting counterpart together (e.g. `welch_moving_average` + `plot_warm_up_diagnostic`) splits into two bullets, one per section, each keeping only the sub-bullets that are actually about that function — cross-reference the other with a short parenthetical (`see New metrics`/`see New plots`) rather than duplicating content.
