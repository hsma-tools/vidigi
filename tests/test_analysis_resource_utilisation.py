...
def test_by_run_capacity_is_nan_when_steps_disagree():
    """Two different steps with different capacities - pooling them under one
    "run" row has no single well-defined capacity, so utilisation must be NaN
    rather than picking one step's capacity arbitrarily."""
    logger = EventLogger(run_number=1)
    logger.log_resource_use_start(
